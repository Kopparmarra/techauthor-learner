...
function makeDrill(spec){
 ...
 if(k==="select"){
   ...
   d.test=()=>{
     ...
     if(name==="find"){
       // Opening Find/Replace is enough (task is open, not replace).
       return a?.kind==="findreplace"||(a?.kind==="shortcut"&&a?.name==="find")||!!ev.findOpened||!!sc.find;
     }
   ...
   // Task is to *find* a tag — opening the dialog alone is not enough; Find Next must hit.
   d.setup=()=>drillSetupBase("para");
   d.test=()=>state.lastLearningAction?.kind==="find-tag"||!!state.drillEvidence?.findTagHit
 }
